...
class Driver(InstrumentDriver.InstrumentWorker):
    ...
    def performSetValue(self, quant, value, sweepRate=0.0, options={}):
        ...
        if quant.name == 'Read file':
        	...
        	self.saved_data = sio.loadmat(file_path)
        	self.setValue("T0", self.saved_data["T0"])
        	self.setValue("High Cutoff Freq.", self.saved_data["High Cutoff Freq."])
        	self.setValue("Power Spectral Density", self.saved_data["Power Spectral Density"])
        	self.setValue("Center Freq.", self.saved_data["Center Freq."])
        	self.setValue("FWHM", self.saved_data["FWHM"])
    ...
    def getWaveformFromMemory(self, quant):
        ...
        dTrace = { 'Trace - Noise (Time-Domain)': self.vNoise_Time, 
                  ...
                  'Trace - Modulated Noise (Time-Domain)': self.vNoise_Time * self.vEnvelope
    ...
    def generateNoise(self):
        ...
        if (str_NoiseType == 'Custom'):
            ...
            index = self.getValue('Noise Index')
            index = np.clip(index, 0, len(self.saved_data['T_noise'])-1)
            vec_T_noise_data = self.saved_data['T_noise'][index]
            if (len(vec_T_noise_data) >= len(vec_time)):
                ...
                for i in range(N_copy):
                    # self.log(i*len(vec_T_noise_data),len(vec_time))
            ...
            vec_F_noise = self.saved_data['F_noise']
            ...
            S0 = self.getValue('Power Spectral Density')
            freq_step = 1/T0
            ...
            if str_NoiseType == 'White':
                ...
                fwidth = self.getValue('FWHM')
                vec_F_noise = lorentz(2 * np.pi * vec_freq , S0= S0, wc = 2 * np.pi * fwidth * 0.5, w0 = 2 * np.pi * f0)
                vec_T_noise = genProcess(vec_time, vec_F_noise, T0)
        ...
        return ({
                 ...
                 "vec_F_noise":vec_F_noise[:len(vec_time)//2],
